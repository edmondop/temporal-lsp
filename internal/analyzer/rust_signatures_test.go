...
func TestRustSignatureAnalyzerDetectsBadSignatures(t *testing.T) {
	a := NewRustSignatureAnalyzer()

	filePath := filepath.Join(testdataDir(), "rust", "bad_signatures.rs")
...
func TestRustSignatureAnalyzerAcceptsGoodSignatures(t *testing.T) {
	a := NewRustSignatureAnalyzer()

	filePath := filepath.Join(testdataDir(), "rust", "good_signatures.rs")
	content, err := os.ReadFile(filePath)
	if err != nil {
		t.Fatalf("failed to read fixture: %v", err)
	}
